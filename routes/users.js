...
import Vote from "../models/vote.js"; // Import your Vote model, adjust path as necessary  
...
router.post("/login", loginValidator, async (req, res, next) => {  
  ...
  try {  
    ...
    const accessToken = jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: "24d" });  
    const refreshToken = jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: "24d" });  
...
router.get("/user/votes", auth, async (req, res) => {  
  try {  
    const votes = await Vote.find({ userId: req.user.id }).lean();  
